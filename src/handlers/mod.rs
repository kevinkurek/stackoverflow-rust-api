...
pub async fn create_question(
    ...
    // .as_ref() required since fields are smart pointers to give &dyn Trait instead of &Arc<_>
    handlers_inner::create_question(question, questions_dao.as_ref())
        .await
        .map(Json)

    // Mock example return for pre-handler work
    // Json(QuestionDetail {
...
pub async fn read_questions(State(AppState { questions_dao, ..}): State<AppState>,
) -> Result<impl IntoResponse, impl IntoResponse> {
