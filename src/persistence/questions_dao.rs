...
impl QuestionsDaoImpl {
    pub fn new(db: PgPool) -> Self {
        Self { db } // return an instance of QuestionsDaoImpl
    }
}
...
impl QuestionsDao for QuestionsDaoImpl {
    async fn create_question(&self, question: Question) -> Result<QuestionDetail, DBError> {
        ...
        Ok(QuestionDetail {
            question_uuid: record.question_uuid.to_string(), // randomly generated, shown in migration setup
            title: record.title,
            description: record.description,
            created_at: record.created_at.to_string(), // created at time of db insert, shown in migration setup
        })
    }

    async fn delete_question(&self, question_uuid: String) -> Result<(), DBError> {

        let parsed_uuid = sqlx::types::Uuid::parse_str(&question_uuid)
        .map_err(|_| {DBError::InvalidUUID(format!("Could not parse question UUID: {}", question_uuid))
        })?;

        sqlx::query!(
            r#"DELETE FROM questions WHERE question_uuid = $1"#, parsed_uuid)
            .execute(&self.db)
            .await
            .map_err(|e| DBError::Other(Box::new(e)))?;
