...
impl AnswersDaoImpl {
    pub fn new(db: PgPool) -> Self {
        Self {db} // return an instance of AnswersDaoImpl
    }
}
...
impl AnswersDao for AnswersDaoImpl {
    async fn create_answer(&self, answer: Answer) -> Result<AnswerDetail, DBError> {
        ...
        let uuid = sqlx::types::Uuid::parse_str(&answer.question_uuid)
            .map_err(|_| {DBError::InvalidUUID(format!(
    ...
    async fn get_answers(&self, question_uuid: String) -> Result<Vec<AnswerDetail>, DBError> {
        ...
        let uuid = sqlx::types::Uuid::parse_str(&question_uuid)
            .map_err(|_| {
                DBError::InvalidUUID(
                    format!("Could not parse question UUID: {}", question_uuid)
                )
            })?;
        ...
        let answers = records
            .iter()
            .map(|f| AnswerDetail {
                answer_uuid: f.answer_uuid.to_string(),
                question_uuid: f.question_uuid.to_string(),
